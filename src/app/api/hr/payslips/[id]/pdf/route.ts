...
//   src/app/api/hr/payslips/[id]/pdf/route.ts
// ============================================================
// Sert le PDF d'un bulletin archivé (bucket privé hr-employee-docs) :
...
export async function GET(req: Request, ctx: { params: { id: string } }) {
  ...
  var res = await sb.from("hr_payslips").select("pdf_path").eq("id", id).single()
  if (res.error || !res.data || !res.data.pdf_path) {
    return new Response("Bulletin introuvable", { status: 404 })
  }

  var su = await sb.storage.from("hr-employee-docs").createSignedUrl(res.data.pdf_path, 300)
