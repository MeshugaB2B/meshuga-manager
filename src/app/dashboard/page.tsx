...
export default function DashboardPage() {
  ...
  async function generateEmail(p) {
    ...
    const pressRefs = 'Paris Premiere (https://www.parispremiere.fr/), Telerama (https://www.telerama.fr/), Konbini (https://www.konbini.com/fr/), Les Echos (https://www.lesechos.fr/), Do It In Paris (https://www.doitinparis.com/), Grazia (https://www.grazia.fr/), Magazine Acumen (https://www.acumen-magazine.fr/), Guide du Fooding (https://lefooding.com/), Tres Tres Bon video (https://www.youtube.com/@TresTresBon)'
    const prompt = 'Tu es ' + senderName + ' pour Meshuga Crazy Deli (Paris 6e, 3 rue Vavin). Restaurant new-yorkais premium. Dans le corps de l\'email, integre naturellement 2-3 liens parmi : ' + pressRefs + ' : pastrami, lobster rolls, sandwichs gastronomiques. Spécialisés dans les plateaux déjeuner B2B et le catering événementiel sur tout Paris.\n\nMeshuga est régulièrement mis en avant par : Paris Première, Télérama, Konbini, Les Echos, Do It In Paris, Grazia, Magazine Acumen. On figure aussi dans le Guide du Fooding et Très Très Bon — cite au moins 3 de ces références dans l\'email pour asseoir notre crédibilité.\n\nÉcris un email de prise de contact pour ce prospect :\n- Entreprise : ' + p.name + '\n- Secteur : ' + (CATS_MAP[p.cat] ? CATS_MAP[p.cat].label : p.cat) + '\n- Localisation : ' + p.arrondissement + '\n- Taille : ' + p.taille + ' employés\n- Ce qu\'on peut proposer : ' + p.type + '\n- Angle : ' + p.pitch + '\n\nSois concis (6-8 lignes), personnalisé, et mets en avant notre Tuna Melt comme signature. Commence par "Objet : " sur la 1ère ligne.\nSignature : ' + senderSig + ' | 3 rue Vavin, Paris 6e'
    try {
