...
export default function DashboardPage() {
  ...
  function generateAndPrintDoc(dv, isFacture) {
    ...
    var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>'+titre+' '+numero+'</title><style>'+
      ...
      'body { font-family: Arial, sans-serif; color: #191923; font-size: 12px; background: white }'+
      ...
      '.header { display: flex; justify-content: space-between; align-items: flex-start; padding-bottom: 16px; border-bottom: 3px solid #FF82D7; margin-bottom: 20px }'+
      '.logo img { height: 60px; width: auto }'+
      '.logo-sub { font-size: 9px; color: #888; margin-top: 4px; text-transform: uppercase; letter-spacing: 1px }'+
      '.doc-info { text-align: right }'+
      '.doc-type { font-size: 26px; font-weight: 900; color: #191923; letter-spacing: 2px }'+
      '.doc-num { font-size: 11px; color: #888; margin-top: 3px }'+
      '.doc-date { font-size: 11px; color: #888 }'+
      '.parties { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px }'+
      '.party-box { background: #F8F8F8; border-radius: 6px; padding: 14px; border-left: 4px solid #FFEB5A }'+
      '.party-box.client { border-left-color: #FF82D7 }'+
      '.party-label { font-size: 8px; font-weight: 900; text-transform: uppercase; letter-spacing: 1.5px; color: #888; margin-bottom: 6px }'+
      ...
      '.event-badge { display: inline-block; background: #FFEB5A; border: 1.5px solid #191923; border-radius: 4px; padding: 3px 8px; font-size: 10px; font-weight: 900; margin-top: 6px }'+
      'table { width: 100%; border-collapse: collapse; margin-bottom: 16px }'+
      'thead th { background: #191923; color: #FFEB5A; padding: 9px 12px; font-size: 9px; text-transform: uppercase; letter-spacing: 1px; font-weight: 900 }'+
      'tbody td { padding: 8px 12px; border-bottom: 1px solid #EBEBEB; font-size: 11px }'+
      'tbody tr:nth-child(even) td { background: #F8F8F8 }'+
      '.strike { text-decoration: line-through; opacity: 0.5 }'+
      '.offert { color: #009D3A; font-weight: 900; font-size: 10px; margin-left: 6px }'+
      '.remise-row td { color: #CC0066; font-weight: 900 }'+
      '.totals-section { display: flex; justify-content: flex-end; margin-bottom: 20px }'+
      '.totals-box { width: 280px; border: 2px solid #191923; border-radius: 6px; overflow: hidden }'+
      '.t-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 14px; border-bottom: 1px solid #EBEBEB; font-size: 12px }'+
      '.t-row.tva { color: #888; font-size: 10px }'+
      '.t-final { display: flex; justify-content: space-between; align-items: center; padding: 12px 14px; background: #FFEB5A }'+
      '.t-final span { font-weight: 900; font-size: 15px; color: #191923 }'+
      '.t-per-person { text-align: center; padding: 5px; font-size: 10px; color: #888; background: #F8F8F8 }'+
      '.rib-box { background: #F8F8F8; border: 2px solid #191923; border-radius: 6px; padding: 14px 18px; margin-bottom: 16px }'+
      '.rib-title { font-size: 9px; font-weight: 900; text-transform: uppercase; letter-spacing: 1.5px; color: #FF82D7; margin-bottom: 10px }'+
      '.rib-grid { display: grid; grid-template-columns: auto auto auto auto; gap: 16px }'+
      '.rib-item label { display: block; font-size: 8px; text-transform: uppercase; letter-spacing: 1px; color: #888; margin-bottom: 2px }'+
      '.rib-item span { font-size: 11px; font-weight: 900; font-family: monospace }'+
      '.notes-box { background: #FFFDE7; border-left: 4px solid #FFEB5A; padding: 10px 14px; margin-bottom: 14px; font-size: 11px }'+
      '.conditions { font-size: 10px; color: #555; margin-bottom: 12px; line-height: 1.6 }'+
      '.legal { font-size: 8px; color: #aaa; padding-top: 10px; border-top: 1px solid #EBEBEB; line-height: 1.6 }'+
      '.pink-footer { background: #FF82D7; color: #191923; text-align: center; padding: 8px; font-size: 9px; font-weight: 900; letter-spacing: 1px; margin-top: 16px; border-radius: 4px }'+
      '.print-btn-wrap { text-align: center; padding: 24px; background: #F8F8F8; border-top: 2px solid #EBEBEB; margin-top: 20px }'+
      '</style></head><body>'+
      '<div class="page">'+
      ...
      '<div class="header">'+
        ...
        '<div class="doc-info">'+
          ...
          '<div class="doc-date">Date : '+new Date().toLocaleDateString('fr-FR')+'</div>'+
          (isFacture ? '<div style="margin-top:6px;background:#191923;color:#FFEB5A;padding:3px 10px;border-radius:3px;font-size:10px;font-weight:900;display:inline-block">ORIGINAL</div>' : '')+
        '</div>'+
      '</div>'+
      ...
      '<div class="parties">'+
        ...
        '<div class="party-box client">'+
          ...
          (dv.event_date||dv.event_lieu ? '<div style="margin-top:8px"><div class="party-detail"><strong>Evenement :</strong> '+(dv.event_date?new Date(dv.event_date).toLocaleDateString('fr-FR'):'')+(dv.event_lieu?' — '+dv.event_lieu:'')+'</div></div>' : '')+
          '<div class="event-badge">'+dv.nb_personnes+' personnes</div>'+
        '</div>'+
      '</div>'+
      
      '<table>'+
        '<thead><tr>'+
          '<th style="width:55%;text-align:left">Designation</th>'+
      ...
      '<div class="totals-section">'+
        '<div class="totals-box">'+
          ...
          '<div class="t-row tva"><span>TVA 5,5%</span><span>'+tva.toFixed(2)+' EUR</span></div>'+
          '<div class="t-final"><span>TOTAL TTC</span><span>'+totalTTC.toFixed(2)+' EUR</span></div>'+
          '<div class="t-per-person">soit '+parseFloat(totalTTC/dv.nb_personnes).toFixed(2)+' EUR TTC / personne</div>'+
        '</div>'+
      '</div>'+
      
      (isFacture ?
        '<div class="rib-box">'+
          '<div class="rib-title">Reglement par virement bancaire — Banque Populaire Rives de Paris</div>'+
          '<div class="rib-grid">'+
            ...
            '<div class="rib-item"><label>Domiciliation</label><span>110 Bld Saint-Germain, 75006 Paris</span></div>'+
          '</div>'+
        '</div>'
      : '')+
      
      (dv.notes ? '<div class="notes-box"><strong>Notes :</strong> '+dv.notes+'</div>' : '')+
      
      '<div class="conditions">'+
        '<strong>Conditions de reglement :</strong> '+(isFacture ? 'Virement bancaire — 30% verse a la commande, solde a regler 72h avant la date de l&#39;evenement.' : '30% a la commande, solde a regler 72h avant la date de l&#39;evenement. Devis valable 30 jours.')+
      '</div>'+
      
      '<div class="legal">'+
        'SAS AEGIA FOOD (enseigne Meshuga) — SASU — Capital social : 1 000 EUR — RCS Paris — SIRET 904 639 531 00014 — Code APE : 56.10C — TVA intracommunautaire : FR31904639531<br/>'+
        (isFacture ? 'Conformement a la loi, tout retard de paiement entraine l&#39;exigibilite de penalites d&#39;un taux egal a 3 fois le taux d&#39;interet legal, ainsi qu&#39;une indemnite forfaitaire de 40 EUR pour frais de recouvrement.' : 'TVA sur les produits alimentaires a taux reduit de 5,5%. Prix exprimes en euros HT. Tout commencement d&#39;execution vaut acceptation du present devis.')+
      '</div>'+
      
      '<div class="pink-footer">meshuga — crazy deli — 3 rue vavin, paris 6e — edward@meshuga.fr</div>'+
      
      '</div>'+
      
      '<div class="no-print print-btn-wrap">'+
        '<p style="margin-bottom:12px;font-size:12px;color:#888">Dans la fenetre d&#39;impression, desactivez les en-tetes/pieds de page et choisissez <strong>Enregistrer en PDF</strong></p>'+
        '<button onclick="window.print()" style="padding:12px 32px;background:#191923;color:#FFEB5A;border:none;border-radius:6px;font-size:14px;font-weight:900;cursor:pointer;letter-spacing:1px">📥 Imprimer / Enregistrer en PDF</button>'+
      '</div>'+
