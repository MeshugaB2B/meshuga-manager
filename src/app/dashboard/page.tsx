...
export default function DashboardPage() {
  ...
  function generateAndPrintDoc(dv, isFacture) {
    ...
    var mep=parseFloat(dv.mep||dv.mise_en_place||0),mepOff=dv.mep_offert||dv.mise_en_place_offert
    var liv=parseFloat(dv.liv||dv.livraison||0),livOff=dv.liv_offert||dv.livraison_offert
    var mepHT=mepOff?mep:mep*(1-(parseFloat(dv.mep_remise||dv.remise_mep_pct||0)/100))
    var remPct=parseFloat(dv.remise_pct||dv.remise_total_pct||0)
    var remMt=parseFloat(dv.remise_montant||0)
    var totalHT=parseFloat(dv.total_ht||0),tva=parseFloat(dv.tva||0),totalTTC=parseFloat(dv.total_ttc||0)
    var numero=isFacture?(dv.facture_numero||dv.numero):dv.numero
    var titre=isFacture?'FACTURE':'DEVIS'
    var mepRow=mep>0?'<tr><td>'+(mepOff?'<span style="text-decoration:line-through;opacity:.4">Mise en place / Show cooking</span> <strong style="color:#009D3A">OFFERT</strong>':'Mise en place / Show cooking')+'</td><td style="text-align:center">1</td><td style="text-align:right">'+(mepOff?'<span style="text-decoration:line-through;opacity:.4">'+mep.toFixed(2)+' EUR</span>':mepHT.toFixed(2)+' EUR')+'</td><td style="text-align:right;font-weight:900">'+(mepOff?'<strong style="color:#009D3A">0,00 EUR</strong>':mepHT.toFixed(2)+' EUR')+'</td></tr>':''
    var livRow=liv>0?'<tr><td>'+(livOff?'<span style="text-decoration:line-through;opacity:.4">Frais de livraison</span> <strong style="color:#009D3A">OFFERT</strong>':'Frais de livraison')+'</td><td style="text-align:center">1</td><td style="text-align:right">'+(livOff?'<span style="text-decoration:line-through;opacity:.4">'+liv.toFixed(2)+' EUR</span>':liv.toFixed(2)+' EUR')+'</td><td style="text-align:right;font-weight:900">'+(livOff?'<strong style="color:#009D3A">0,00 EUR</strong>':liv.toFixed(2)+' EUR')+'</td></tr>':''
    var remRow=remMt>0?'<tr><td style="color:#CC0066;font-weight:900">Remise commerciale ('+remPct+'%)</td><td></td><td></td><td style="text-align:right;color:#CC0066;font-weight:900">-'+remMt.toFixed(2)+' EUR</td></tr>':''
          ...
          {page === 'dash' && (
            <div>
              ...
              <div className="card" style={{padding:0,overflow:'hidden',marginBottom:10}}>
                <div style={{background:'#FF82D7',padding:'10px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8,borderBottom:'2px solid #191923'}}>
                  <div className="yt" style={{fontSize:17,color:'#191923'}}>Zelty &mdash; Caisse</div>
                  <div style={{display:'flex',gap:4,alignItems:'center'}}>
                    {['day','week','month','year'].map(function(p) {
                      var labels = {day:'Auj.', week:'Sem.', month:'Mois', year:'Année'}
                      return (<div key={p} onClick={function() { setZeltyPeriod(p) }} style={{padding:'4px 10px',borderRadius:4,border:'2px solid #191923',background:zeltyPeriod===p?'#FFEB5A':'#FFFFFF',color:'#191923',fontSize:10,fontWeight:900,cursor:'pointer',textTransform:'uppercase'}}>{labels[p]}</div>)
                    })}
                ...
                {zeltyLoading && <div style={{padding:'18px',textAlign:'center',opacity:.3,fontSize:11,fontWeight:900}}>Chargement...</div>}
                  ...
                  <div>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',borderBottom:'1px solid #EBEBEB'}}>
                      {[{icon:'&#128176;',v:zeltyCA,l:'CA'},{icon:'&#129534;',v:zeltyTickets,l:'Tickets'},{icon:'&#128722;',v:zeltyAvg,l:'Panier moyen'},{icon:'&#128200;',v:zeltyEvol,l:'vs préc.',color:zeltyEvolColor}].map(function(k,i) {
                        return (<div key={i} style={{padding:'12px 14px',borderRight:i<3?'1px solid #EBEBEB':'none'}}><div style={{fontSize:16,marginBottom:3}} dangerouslySetInnerHTML={{__html:k.icon}} /><div style={{fontWeight:900,fontSize:20,lineHeight:1,color:k.color||'#191923'}}>{k.v}</div><div style={{fontSize:9,opacity:.4,textTransform:'uppercase',marginTop:3}}>{k.l}</div></div>)
                      })}
                    </div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr'}}>
                      <div style={{padding:'12px 14px',borderRight:'1px solid #EBEBEB'}}>
                        <div className="yt" style={{fontSize:14,marginBottom:8}}>Top produits</div>
                        {zeltyData.topProducts && zeltyData.topProducts.length > 0 ? zeltyData.topProducts.slice(0,5).map(function(p,i) {
                          ...
                          return (<div key={p.name} style={{marginBottom:6}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:2}}><span style={{fontSize:10,fontWeight:i===0?900:400}}>{i+1}. {p.name}</span><span style={{fontSize:9,fontWeight:900,background:'#FFEB5A',padding:'1px 5px',borderRadius:3,border:'1px solid #191923'}}>{p.qty}x</span></div><div style={{height:3,background:'#EBEBEB',borderRadius:2}}><div style={{height:'100%',background:i===0?'#FF82D7':'#191923',borderRadius:2,width:Math.round(p.qty/mx*100)+'%',opacity:i===0?1:0.25}} /></div></div>)
                        }) : <div style={{fontSize:10,opacity:.3}}>Aucune donnée</div>}
                      </div>
                      <div style={{padding:'12px 14px'}}>
                        <div className="yt" style={{fontSize:14,marginBottom:8}}>Heures de pointe</div>
                        {zeltyData.peakHours && zeltyData.peakHours.filter(function(h){return h.count>0}).length > 0 ? zeltyData.peakHours.filter(function(h){return h.count>0}).slice(0,7).map(function(h,i) {
                          ...
                          return (<div key={h.hour} style={{display:'flex',alignItems:'center',gap:8,marginBottom:5}}><span style={{fontSize:9,opacity:.4,width:26,flexShrink:0}}>{h.hour}h</span><div style={{flex:1,height:6,background:'#EBEBEB',borderRadius:2}}><div style={{height:'100%',background:i===0?'#FF82D7':'#FFEB5A',borderRadius:2,width:Math.round(h.count/mx*100)+'%'}} /></div><span style={{fontSize:9,fontWeight:900,width:16,textAlign:'right'}}>{h.count}</span></div>)
...
{!isEmy && reports.length > 0 && (
          ...
          {page === 'crm' && (
            <div>
              ...
              {(function() {
                ...
                var caBH = devisAcceptes.reduce(function(s,d){return s+parseFloat(d.total_ht||0)},0)
                var devisEnAttente = devisList.filter(function(d){return d.statut==='envoye'||d.statut==='brouillon'})
                var devisAttenteMontant = devisEnAttente.reduce(function(s,d){return s+parseFloat(d.total_ttc||0)},0)
                var commissionEmy = caBH * 0.10
                  ...
                  <div>
                    <div className="g4" style={{marginBottom:10}}>
                      <div className="kc" style={{background:'#FFFFFF'}}>
                        ...
                        <div className="kv">{caBH.toFixed(0)}€</div>
                        <div style={{fontSize:9,opacity:.4,marginTop:2}}>HT devis acceptés</div>
                        <div className="ki">&#128176;</div>
                      </div>
                      <div className="kc" style={{background:'#FFFFFF'}} onClick={function(){nav('devis')}}>
                        <div className="kl">Devis en attente</div>
                        <div className="kv">{devisAttenteMontant.toFixed(0)}€</div>
                        <div style={{fontSize:9,opacity:.4,marginTop:2}}>{devisEnAttente.length} devis TTC</div>
                        <div className="ki">&#128196;</div>
                      </div>
                      <div className="kc" style={{background:'#FFFFFF'}}>
                        <div className="kl">Commission Emy</div>
                        <div className="kv">{commissionEmy.toFixed(0)}€</div>
                        <div style={{fontSize:9,opacity:.4,marginTop:2}}>10% du CA HT signé</div>
                        <div className="ki">&#11088;</div>
                      </div>
                      <div className="kc" style={{background:urgents.length>0?'#FFF0F5':'#FFFFFF'}}>
                        <div className="kl">Relances urgentes</div>
                        <div className="kv" style={{color:urgents.length>0?'#CC0066':'#191923'}}>{urgents.length}</div>
                        <div style={{fontSize:9,opacity:.4,marginTop:2}}>{prospects.filter(function(p){return p.status!=='won'&&p.status!=='lost'}).length} en pipeline</div>
                        <div className="ki">&#128276;</div>
                      </div>
                    </div>
                    {urgents.length > 0 && (
                      <div className="card-p" style={{marginBottom:10,border:'2px solid #191923',boxShadow:'3px 3px 0 #191923'}}>
                        ...
                        {urgents.slice(0,4).map(function(p) {
                          return (
                            <div key={p.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:'1px solid rgba(25,25,35,.1)'}}>
                              <div>
                                <div style={{fontWeight:900,fontSize:13}}>{p.name}</div>
                                <div style={{fontSize:11,opacity:.7}}>{p.nextAction}</div>
                              </div>
                              <button className="btn btn-sm" style={{background:'#191923',color:'#FFEB5A'}} onClick={function(){generateEmail({id:p.id,name:p.name,cat:'crm',arrondissement:'',taille:p.size,pitch:p.notes||'',type:p.category,email:p.email})}}>Email IA</button>
                            </div>
                          )
                        })}
                      </div>
                    ...
                    {devisEnAttente.length > 0 && (
                      <div className="card" style={{marginBottom:10}}>
                        <div className="ct">Devis en attente de réponse</div>
                        {devisEnAttente.slice(0,4).map(function(dv) {
                          return (
                            <div key={dv.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:'1px solid #EBEBEB'}}>
                              <div>
                                <div style={{fontWeight:900,fontSize:12}}>{dv.client_nom}</div>
                                <div style={{fontSize:10,opacity:.4}}>{dv.numero} · {dv.event_date?new Date(dv.event_date).toLocaleDateString('fr-FR'):''}</div>
                              </div>
                              <div style={{textAlign:'right'}}>
                                <div style={{fontWeight:900,fontSize:12}}>{parseFloat(dv.total_ttc||0).toFixed(0)}€ TTC</div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:6,marginBottom:10}}>
                {['to_contact','contacted','nego','won','lost'].map(function(s) {
                  var labels={to_contact:'A contacter',contacted:'Contacté',nego:'Négo',won:'Gagné',lost:'Perdu'}
                  var cols={to_contact:'#888',contacted:'#B8920A',nego:'#005FFF',won:'#009D3A',lost:'#CC0066'}
                  var cnt=prospects.filter(function(p){return p.status===s}).length
                  return (
                    <div key={s} style={{textAlign:'center',padding:'8px 4px',border:'2px solid '+cols[s],borderRadius:5,background:'#fff',cursor:'pointer'}} onClick={function(){setCrmFilter(s)}}>
                      <div style={{fontWeight:900,fontSize:18,color:cols[s]}}>{cnt}</div>
                      <div style={{fontSize:8,fontWeight:900,textTransform:'uppercase',color:cols[s]}}>{labels[s]}</div>
                    </div>
                  )
                })}
              </div>
              ...
              <div style={{display:'flex',gap:8,marginBottom:10,flexWrap:'wrap'}}>
                ...
                <select className="inp" style={{width:140}} value={crmFilter} onChange={function(e){setCrmFilter(e.target.value)}}>
              ...
              {prospects.filter(function(p){
                var mf=!crmFilter||crmFilter==='all'||p.status===crmFilter
                var ms=!crmSearch||p.name.toLowerCase().indexOf(crmSearch.toLowerCase())>=0||(p.email&&p.email.toLowerCase().indexOf(crmSearch.toLowerCase())>=0)
                return mf&&ms
              }).map(function(p) {
                var dvP=devisList.filter(function(d){return d.client_nom===p.name||String(d.prospect_id)===String(p.id)})
                  ...
                  <div key={p.id} className="card" style={{cursor:'pointer',marginBottom:8,borderLeft:'4px solid '+(isLate?'#CC0066':STATUS_PC[p.status])}} onClick={function(){openModal('prospect',Object.assign({},p))}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10,marginBottom:6}}>
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:11,opacity:.4}}>{p.category} · {p.email}</div>
                        {p.nextDate&&<div style={{fontSize:11,marginTop:3,color:isLate?'#CC0066':'#191923',fontWeight:isLate?900:400}}>{isLate?'En retard : ':''}{p.nextAction} — {new Date(p.nextDate).toLocaleDateString('fr-FR')}</div>}
                      </div>
                    ...
                    {dvP.length>0&&<div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:6}}>
                      {dvP.slice(0,3).map(function(dv){
                        ...
                        return <span key={dv.id} style={{fontSize:9,fontWeight:900,padding:'2px 6px',border:'1.5px solid '+(sc[dv.statut]||'#888'),borderRadius:3,color:sc[dv.statut]||'#888'}}>
                          {dv.numero} — {parseFloat(dv.total_ttc||0).toFixed(0)}€ · {sl[dv.statut]||dv.statut}
                        </span>
                      })}
                    </div>}
                    {p.notes&&<div style={{fontSize:11,opacity:.4,marginBottom:6,fontStyle:'italic'}}>"{p.notes.slice(0,80)}{p.notes.length>80?'...':''}"</div>}
                    <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                      <button className="btn btn-p btn-sm" onClick={function(e){e.stopPropagation();generateEmail({id:p.id,name:p.name,cat:'crm',arrondissement:'',taille:p.size,pitch:p.notes||'',type:p.category,email:p.email})}}>Email IA</button>
          ...
          {page === 'devis' && (
            <div>
              ...
              {devisView === 'list' && (
                <div>
                  {devisList.length === 0 && <div className="card" style={{textAlign:'center',padding:40,opacity:.4}}><div style={{fontSize:36,marginBottom:8}}>&#128196;</div><div style={{fontWeight:900}}>Aucun devis</div></div>}
                      ...
                      <div key={dv.id} className="card" style={{marginBottom:8}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10,flexWrap:'wrap'}}>
                          <div style={{flex:1}}>
                            ...
                            <div style={{fontSize:11,opacity:.4}}>{dv.event_date?new Date(dv.event_date).toLocaleDateString('fr-FR'):''} &middot; {dv.event_lieu} &middot; {dv.nb_personnes} pers.</div>
                          </div>
                        ...
                        <div style={{display:'flex',gap:6,marginTop:10,flexWrap:'wrap'}}>
                          ...
                          {dv.statut==='envoye' && <span>
                            <button className="btn btn-sm" style={{background:'#009D3A',color:'#fff',marginRight:4}} onClick={function(){updateDevisStatut(dv.id,'accepte','')}}>Accepté</button>
                            <button className="btn btn-red btn-sm" style={{marginRight:4}} onClick={function(){updateDevisStatut(dv.id,'refuse','')}}>Refusé</button>
                          ...
                          {dv.facture_numero && <button className="btn btn-n btn-sm" onClick={function(){generateAndPrintDoc(dv,true)}}>PDF Facture</button>}
                        </div>
                      </div>
                    )
                  })}
                ...
                <div className="g2">
                  <div>
                    ...
                    <div className="card" style={{marginBottom:10}}>
                      ...
                      <div style={{padding:'10px',border:'1.5px solid #EBEBEB',borderRadius:5,marginBottom:8}}>
                        ...
                        <div style={{display:'flex',gap:8}}><input type="number" className="inp" style={{flex:1}} value={devisMiseEnPlace} onChange={function(e){setDevisMiseEnPlace(parseFloat(e.target.value)||0)}} disabled={devisMepOffert} /><input type="number" className="inp" style={{width:60}} placeholder="%" value={devisMiseEnPlaceRemise} min="0" max="100" onChange={function(e){setDevisMiseEnPlacePct(parseFloat(e.target.value)||0)}} disabled={devisMepOffert} /></div>
                      </div>
                      <div style={{padding:'10px',border:'1.5px solid #EBEBEB',borderRadius:5,marginBottom:8}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                      ...
                      <div className="fg"><label className="lbl">Remise total (%)</label><input type="number" className="inp" value={devisRemiseTotal} min="0" max="100" onChange={function(e){setDevisRemiseTotal(parseFloat(e.target.value)||0)}} /></div>
                    </div>
                  ...
                  <div>
                    <div className="card" style={{marginBottom:10}}>
                      ...
                      <div style={{fontSize:10,opacity:.4,marginBottom:8}}>Conseillé : {devisFormat==='normal'?devisNbPersonnes:(devisNbPersonnes*3)} pièces</div>
                        ...
                        return (
                          <div key={s.id} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 0',borderBottom:'1px solid #EBEBEB'}}>
                            ...
                            <div style={{display:'flex',alignItems:'center',gap:5}}>
                              ...
                              <input type="number" min="0" style={{width:44,textAlign:'center',border:'2px solid #191923',borderRadius:4,padding:'3px',fontSize:12,fontWeight:900}} value={qty} onChange={function(e){setQty(parseInt(e.target.value)||0)}} />
                      ...
                      <div style={{marginTop:8,padding:'6px 10px',background:'#FFEB5A',borderRadius:4,border:'1.5px solid #191923',display:'flex',justifyContent:'space-between'}}>
                        <span style={{fontSize:11,fontWeight:900}}>Total</span><span style={{fontSize:13,fontWeight:900}}>{devisItems.reduce(function(s,x){return s+x.qte},0)} pièces</span>
                      </div>
                    </div>
                    {(function() {
                      ...
                      var sousTotal=sandTotal+mepHT+(devisLivraisonOffert?devisLivraison:devisLivraison)
                      var remMt=sousTotal*devisRemiseTotal/100
                      var totalHT=sousTotal-remMt; var tva=totalHT*0.055; var totalTTC=totalHT+tva
                      return (
                        <div className="card" style={{border:'3px solid #191923',boxShadow:'4px 4px 0 #191923'}}>
                          ...
                          <div style={{marginBottom:8}}>
                            ...
                            {devisMiseEnPlace>0 && <div style={{display:'flex',justifyContent:'space-between',padding:'5px 8px',borderBottom:'1px solid #EBEBEB',background:'#FAFAFA',fontSize:11}}><span style={{textDecoration:devisMepOffert?'line-through':'none',opacity:devisMepOffert?.5:1}}>Mise en place{devisMepOffert?' — OFFERT':''}</span><span style={{fontWeight:900,textDecoration:devisMepOffert?'line-through':'none',opacity:devisMepOffert?.5:1}}>{devisMiseEnPlace.toFixed(2)} EUR</span></div>}
                            {devisLivraison>0 && <div style={{display:'flex',justifyContent:'space-between',padding:'5px 8px',borderBottom:'1px solid #EBEBEB',background:'#FAFAFA',fontSize:11}}><span style={{textDecoration:devisLivraisonOffert?'line-through':'none',opacity:devisLivraisonOffert?.5:1}}>Livraison{devisLivraisonOffert?' — OFFERT':''}</span><span style={{fontWeight:900,textDecoration:devisLivraisonOffert?'line-through':'none',opacity:devisLivraisonOffert?.5:1}}>{devisLivraison.toFixed(2)} EUR</span></div>}
                            {remMt>0 && <div style={{display:'flex',justifyContent:'space-between',padding:'5px 8px',background:'#FFF0F5',fontSize:11,color:'#CC0066'}}><span>Remise ({devisRemiseTotal}%)</span><span style={{fontWeight:900}}>-{remMt.toFixed(2)} EUR</span></div>}
                          </div>
                          ...
                          <div style={{display:'flex',justifyContent:'space-between',padding:'10px 8px',background:'#FFEB5A',borderRadius:5,marginTop:8,border:'2px solid #191923'}}>
                            <span className="yt" style={{fontSize:20}}>Total TTC</span><span style={{fontWeight:900,fontSize:18}}>{totalTTC.toFixed(2)} EUR</span>
                          </div>
          ...
          {page === 'journal' && !isEmy && (
            <div>
              <div className="ph">
                <div><div className="pt">Journal d'Emy</div><div className="ps">Sessions · Contacts · Emails</div></div>
                <button className="btn btn-y btn-sm" onClick={function(){
                  sb().from('activity_log').select('*').order('created_at', {ascending:false}).limit(200).then(function(r){if(r.data) setActivityLog(r.data)})
                  toast('Journal actualisé !')
                }}>Actualiser</button>
              </div>
              <div className="card" style={{marginBottom:12}}>
                <div className="yt" style={{fontSize:15,marginBottom:8,color:'#FF82D7'}}>Sessions de connexion</div>
                {activityLog.filter(function(a) { return a.type === 'session_start' }).slice(0,10).map(function(a, i) {
                  return (
                    <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,.1)'}}>
                      ...
                      <div style={{fontSize:11,opacity:.4}}>{new Date(a.created_at).toLocaleDateString('fr-FR',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})}</div>
                    </div>
                  )
                })}
