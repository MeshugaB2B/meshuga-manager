...
export default function DashboardPage() {
  ...
  function saveDevisToSupabase(payload, callback) {
    var isNew = !payload.id
    if (isNew) {
      sb().from('devis').insert(payload).select().then(function(r) {
        if (r.data && r.data[0]) { loadDevis(); if (callback) callback(r.data[0]) }
      })
    } else {
      var id = payload.id
      var p = Object.assign({}, payload)
      delete p.id; delete p.created_at
      sb().from('devis').update(p).eq('id', id).then(function() { loadDevis(); if (callback) callback(payload) })
    }
  }
  ...
  function generateAndPrintDoc(dv, isFacture) {
    var items = (dv.items||[]).map(function(x) {
      return '<tr><td>'+x.nom+'</td><td style="text-align:center">'+x.qte+'</td><td style="text-align:right">'+parseFloat(x.prix||x.pu_ht||0).toFixed(2)+' EUR</td><td style="text-align:right;font-weight:900">'+parseFloat(x.total_ht||0).toFixed(2)+' EUR</td></tr>'
    }).join('')
    ...
    var remiseRow2 = remiseMontant > 0 ? '<tr><td style="color:#CC0066;font-weight:900">Remise commerciale ('+remisePct+'%)</td><td></td><td></td><td style="text-align:right;color:#CC0066;font-weight:900">-'+remiseMontant.toFixed(2)+' EUR</td></tr>' : ''
    ...
    var css = [
      ...
      'body { font-family: Arial, sans-serif; color: #191923; font-size: 11px; background: white }',
      '@page { size: A4; margin: 0mm }',
      '@media print { html { -webkit-print-color-adjust: exact; print-color-adjust: exact } .no-print { display:none !important } }',
      '.page { width:210mm; min-height:297mm; padding:14mm 16mm 0 16mm; display:flex; flex-direction:column }',
      '.content { flex:1 }',
      '.yt { font-family: Yellowtail, cursive }',
      '.header { display:flex; justify-content:space-between; align-items:flex-start; padding-bottom:12px; border-bottom:3px solid #FF82D7; margin-bottom:16px }',
      '.logo img { height:52px; width:auto }',
      '.logo-sub { font-size:8px; color:#aaa; margin-top:3px }',
      '.doc-type { font-family:Yellowtail,cursive; font-size:38px; color:#191923; text-align:right; line-height:1 }',
      '.doc-num { font-size:10px; color:#888; text-align:right; margin-top:3px }',
      '.parties { display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:16px }',
      '.party { background:#F8F8F8; border-radius:5px; padding:11px 13px; border-left:3px solid #FFEB5A }',
      '.party.client { border-left-color:#FF82D7 }',
      '.party-label { font-family:Yellowtail,cursive; font-size:14px; color:#888; margin-bottom:5px }',
      '.party-name { font-size:13px; font-weight:900; margin-bottom:3px }',
      '.party-detail { font-size:9px; color:#555; margin-top:1px; line-height:1.5 }',
      '.badge { display:inline-block; background:#FFEB5A; border:1.5px solid #191923; border-radius:3px; padding:2px 8px; font-family:Yellowtail,cursive; font-size:13px; margin-top:6px }',
      'table { width:100%; border-collapse:collapse; margin-bottom:14px }',
      'thead th { padding:8px 10px; font-size:8px; text-transform:uppercase; letter-spacing:1px; font-weight:900; color:#191923; border-top:2px solid #191923; border-bottom:2px solid #191923 }',
      'tbody td { padding:7px 10px; border-bottom:1px solid #EBEBEB; font-size:10.5px }',
      'tr:nth-child(even) td { background:#FAFAFA }',
      '.totals-wrap { display:flex; justify-content:flex-end; margin-bottom:14px }',
      '.totals { width:260px }',
      '.t-row { display:flex; justify-content:space-between; padding:5px 0; border-bottom:1px solid #EBEBEB; font-size:11px }',
      '.t-row.gray { color:#999; font-size:9.5px }',
      '.t-final { display:flex; justify-content:space-between; align-items:center; padding:9px 12px; background:#FFEB5A; border:2px solid #191923; border-radius:4px; margin-top:6px }',
      '.t-final .lbl { font-family:Yellowtail,cursive; font-size:22px; color:#191923 }',
      '.t-final .amt { font-weight:900; font-size:15px; color:#191923 }',
      '.per-person { text-align:right; font-size:9px; color:#aaa; margin-top:3px }',
      '.rib { border:1.5px solid #191923; border-radius:5px; padding:12px 14px; margin-bottom:14px; background:#FAFAFA }',
      '.rib-title { font-family:Yellowtail,cursive; font-size:17px; color:#FF82D7; margin-bottom:8px }',
      '.rib-grid { display:grid; grid-template-columns:1fr 1fr 1fr 1fr; gap:10px }',
      '.rib-item label { display:block; font-size:7px; text-transform:uppercase; letter-spacing:1px; color:#aaa; margin-bottom:2px }',
      '.rib-item span { font-size:10px; font-weight:900; font-family:monospace }',
      '.notes { background:#FFFDE7; border-left:3px solid #FFEB5A; padding:8px 12px; margin-bottom:12px; font-size:10px }',
      '.cond-title { font-family:Yellowtail,cursive; font-size:14px; margin-bottom:2px }',
      '.cond { font-size:9.5px; color:#555; margin-bottom:14px; line-height:1.6 }',
      '.footer { padding:12px 0 8px 0; border-top:1px solid #EBEBEB; margin-top:auto }',
      '.legal { font-size:7px; color:#ccc; line-height:1.7; margin-bottom:8px }',
      '.pink-bar { background:#FF82D7; padding:7px 14px; border-radius:4px; text-align:center; font-family:Yellowtail,cursive; font-size:16px; color:#191923 }',
    ].join(' ')

    var html = '<!DOCTYPE html><html><head>'+
      '<meta charset="UTF-8">'+
      '<link href="https://fonts.googleapis.com/css2?family=Yellowtail&display=swap" rel="stylesheet">'+
      '<title>'+titre+' '+numero+'</title><style>'+css+'</style></head><body>'+
      '<div class="page"><div class="content">'+
      '<div class="header">'+
        '<div class="logo"><img src="'+logoSrc+'" alt="meshuga"><div class="logo-sub">Crazy Deli &nbsp;&middot;&nbsp; 3 rue Vavin, 75006 Paris</div></div>'+
        '<div><div class="doc-type">'+titre+'</div><div class="doc-num">N&deg; '+numero+'</div><div class="doc-num">'+new Date().toLocaleDateString('fr-FR')+'</div></div>'+
      '</div>'+
      '<div class="parties">'+
        '<div class="party"><div class="party-label">Emetteur</div><div class="party-name">SAS AEGIA FOOD</div><div class="party-detail">Enseigne : Meshuga Crazy Deli</div><div class="party-detail">3 rue Vavin, 75006 Paris</div><div class="party-detail">SIRET : 904 639 531 00014</div><div class="party-detail">TVA : FR31904639531</div><div class="party-detail">hello@meshuga.fr</div></div>'+
        '<div class="party client"><div class="party-label">Client</div><div class="party-name">'+dv.client_nom+'</div>'+(dv.client_contact?'<div class="party-detail">'+dv.client_contact+'</div>':'')+(dv.client_email?'<div class="party-detail">'+dv.client_email+'</div>':'')+((dv.event_date||dv.event_lieu)?'<div class="party-detail" style="margin-top:5px"><strong>Ev&eacute;nement :</strong> '+(dv.event_date?new Date(dv.event_date).toLocaleDateString('fr-FR'):'')+(dv.event_lieu?' &mdash; '+dv.event_lieu:'')+'</div>':'')+'<div class="badge">'+dv.nb_personnes+' personnes</div></div>'+
      '</div>'+
      '<table><thead><tr><th style="text-align:left;width:52%">D&eacute;signation</th><th style="text-align:center;width:10%">Qte</th><th style="text-align:right;width:19%">PU HT</th><th style="text-align:right;width:19%">Total HT</th></tr></thead><tbody>'+items+mepRow+livRow+remiseRow2+'</tbody></table>'+
      '<div class="totals-wrap"><div class="totals"><div class="t-row"><span>Total HT</span><span style="font-weight:900">'+totalHT.toFixed(2)+' EUR</span></div><div class="t-row gray"><span>TVA 5,5%</span><span>'+tva.toFixed(2)+' EUR</span></div><div class="t-final"><span class="lbl">Total TTC</span><span class="amt">'+totalTTC.toFixed(2)+' EUR</span></div><div class="per-person">soit '+parseFloat(totalTTC/dv.nb_personnes).toFixed(2)+' EUR TTC / personne</div></div></div>'+
      '<div class="rib"><div class="rib-title">'+(isFacture?'R&egrave;glement par virement bancaire':'Coordonn&eacute;es bancaires pour l&#39;acompte')+'</div><div class="rib-grid"><div class="rib-item"><label>Titulaire</label><span>SAS AEGIA FOOD</span></div><div class="rib-item"><label>Banque</label><span>Banque Populaire</span></div><div class="rib-item"><label>IBAN</label><span>FR76 1020 7000 8723 2175 3218 077</span></div><div class="rib-item"><label>BIC</label><span>CCBPFRPPMTG</span></div></div></div>'+
      (dv.notes?'<div class="notes"><strong>Notes :</strong> '+dv.notes+'</div>':'')+
      '<div class="cond-title">Conditions de r&egrave;glement</div><div class="cond">'+(isFacture?'Virement bancaire &mdash; 30% &agrave; la commande, solde 72h avant l&#39;&eacute;v&eacute;nement.':'30% &agrave; la commande, solde 72h avant l&#39;&eacute;v&eacute;nement. Devis valable 30 jours.')+'</div>'+
      '</div>'+
      '<div class="footer"><div class="legal">SAS AEGIA FOOD (enseigne Meshuga) &mdash; SASU &mdash; Capital social : 1 000 EUR &mdash; RCS Paris &mdash; SIRET 904 639 531 00014 &mdash; Code APE : 56.10C &mdash; TVA intracommunautaire : FR31904639531 &mdash; 3 rue Vavin, 75006 Paris<br>'+(isFacture?'Conform&eacute;ment &agrave; la loi, tout retard de paiement entra&icirc;ne l&#39;exigibilit&eacute; de p&eacute;nalit&eacute;s d&#39;un taux &eacute;gal &agrave; 3 fois le taux d&#39;int&eacute;r&ecirc;t l&eacute;gal, ainsi qu&#39;une indemnit&eacute; forfaitaire de 40 EUR pour frais de recouvrement.':'TVA sur les produits alimentaires &agrave; taux r&eacute;duit de 5,5%. Prix HT en euros. Tout commencement d&#39;ex&eacute;cution vaut acceptation du pr&eacute;sent devis.')+'</div><div class="pink-bar">meshuga &mdash; crazy deli &mdash; 3 rue vavin, paris 6e &mdash; hello@meshuga.fr</div></div>'+
      '</div>'+
      '<div class="no-print" style="text-align:center;padding:20px;background:#F8F8F8;border-top:2px solid #EBEBEB">'+
        '<p style="margin-bottom:10px;font-size:11px;color:#888">Dans la fen&ecirc;tre d&#39;impression : d&eacute;cochez <strong>En-t&ecirc;tes et pieds de page</strong> puis <strong>Enregistrer en PDF</strong></p>'+
        '<button onclick="document.fonts.ready.then(function(){window.print()})" style="padding:11px 28px;background:#191923;color:#FFEB5A;border:none;border-radius:5px;font-size:13px;font-weight:900;cursor:pointer">&#128229; Imprimer / PDF</button>'+
      '</div>'+
      '</body></html>'
            ...
            <div>
              ...
              <div className="card" style={{padding:0,overflow:'hidden',marginBottom:10}}>
                <div style={{background:'#FF82D7',padding:'10px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8,borderBottom:'2px solid #191923'}}>
                  <div className="yt" style={{fontSize:17,color:'#191923'}}>Zelty — Caisse</div>
                ...
                {!zeltyLoading && zeltyData && zeltyData.error && <div style={{padding:'12px 16px',fontSize:11,fontWeight:900,opacity:.6}}>{zeltyData.error}</div>}
                          ...
                          return (
                            <div key={p.name} style={{marginBottom:6}}>
                              ...
                              <div style={{height:3,background:'#EBEBEB',borderRadius:2}}>
                                <div style={{height:'100%',background:i===0?'#FF82D7':'#191923',borderRadius:2,width:Math.round(p.qty/mx*100)+'%',opacity:i===0?1:0.25}} />
                              </div>
                            </div>
                          )
                      ...
                      <div key={dv.id} className="card" style={{marginBottom:8}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10,flexWrap:'wrap'}}>
                          <div style={{flex:1}}>
                            ...
                            <div style={{fontSize:11,opacity:.4}}>{dv.event_date ? new Date(dv.event_date).toLocaleDateString('fr-FR') : ''} · {dv.event_lieu} · {dv.nb_personnes} pers.</div>
                          </div>
                        ...
                        <div style={{display:'flex',gap:6,marginTop:10,flexWrap:'wrap'}}>
                          ...
                          {dv.statut==='facture' && dv.paiement_statut!=='paye' && <button className="btn btn-sm" style={{background:'#009D3A',color:'#fff'}} onClick={function(){
                            sb().from('devis').update({paiement_statut:'paye',statut:'paye',solde_recu:true,solde_date:new Date().toISOString().split('T')[0]}).eq('id',dv.id).then(function(){loadDevis();toast('💰 Paiement enregistré !')})
                          }}>💰 Payé</button>}
                          <button className="btn btn-sm" onClick={function(){generateAndPrintDoc(dv,false)}}>📄 PDF</button>
                          {dv.facture_numero && <button className="btn btn-n btn-sm" onClick={function(){generateAndPrintDoc(dv,true)}}>🧾 Facture</button>}
                        </div>
                      </div>
                    )
                  })}
                  ...
                  <div className="g2">
                    <div>
                      ...
                      <div className="card" style={{marginBottom:10}}>
                        ...
                        <div style={{background:'#FFEB5A',border:'1.5px solid #191923',borderRadius:4,padding:'5px 8px',fontSize:10}}>
                          Conseillé : {devisFormat==='normal'?devisNbPersonnes:(devisNbPersonnes*3)} pièces
                        </div>
                      </div>
                      ...
                      <div className="card">
                        ...
                        <textarea className="inp" value={devisNotes} onChange={function(e){setDevisNotes(e.target.value)}} placeholder="Conditions spéciales..." style={{minHeight:70}} />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="card" style={{marginBottom:10}}>
                        ...
                        <div style={{fontSize:10,opacity:.4,marginBottom:8,padding:'4px 8px',background:'#F8F8F8',borderRadius:4}}>
                          Conseillé : {devisFormat==='normal'?devisNbPersonnes:(devisNbPersonnes*3)} pièces
                        </div>
                          ...
                          return (
                            <div key={s.id} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 0',borderBottom:'1px solid #EBEBEB'}}>
                              <div style={{flex:1}}>
                                ...
                                <div style={{fontSize:9,opacity:.4}}>{s.prix.toFixed(2)} EUR HT</div>
                              </div>
                        ...
                        return (
                          <div className="card" style={{border:'3px solid #191923',boxShadow:'4px 4px 0 #191923'}}>
                            ...
                            <div style={{borderRadius:5,overflow:'hidden',marginBottom:8}}>
                              ...
                              {devisMiseEnPlace>0&&<div style={{display:'flex',justifyContent:'space-between',padding:'5px 8px',borderBottom:'1px solid #EBEBEB',background:'#FAFAFA',fontSize:11}}><span style={{textDecoration:devisMepOffert?'line-through':'none',opacity:devisMepOffert?.5:1}}>Mise en place{devisMepOffert&&<span style={{color:'#009D3A',marginLeft:6}}>OFFERT</span>}</span><span style={{fontWeight:900,textDecoration:devisMepOffert?'line-through':'none',opacity:devisMepOffert?.5:1}}>{devisMiseEnPlace.toFixed(2)} EUR</span></div>}
                              {devisLivraison>0&&<div style={{display:'flex',justifyContent:'space-between',padding:'5px 8px',borderBottom:'1px solid #EBEBEB',background:'#FAFAFA',fontSize:11}}><span style={{textDecoration:devisLivraisonOffert?'line-through':'none',opacity:devisLivraisonOffert?.5:1}}>Livraison{devisLivraisonOffert&&<span style={{color:'#009D3A',marginLeft:6}}>OFFERT</span>}</span><span style={{fontWeight:900,textDecoration:devisLivraisonOffert?'line-through':'none',opacity:devisLivraisonOffert?.5:1}}>{devisLivraison.toFixed(2)} EUR</span></div>}
