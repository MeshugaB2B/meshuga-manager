    ...
    supabase.auth.getUser().then(({data:{user}}) => {
      if (user) {
        supabase.from('profiles').select('*').eq('id', user.id).single().then(({data}) => {
          setProfile(data || {role:'edward',full_name:'Edward',email:user.email})
        })
      }
    })
  }, [])
    ...
    try {
      ...
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages:[{
            role:'user',
            content:`Tu es Emy, B2B Manager pour Meshuga Crazy Deli (Paris 6e, 3 rue Vavin). Restaurant new-yorkais premium : pastrami, lobster rolls, sandwichs gastronomiques. Spécialisés dans les plateaux déjeuner B2B et le catering événementiel sur tout Paris.
...
Signature : Emy | B2B Manager Meshuga | emy@meshuga.fr | 3 rue Vavin, Paris 6e`
          }]
        })
      })
      ...
      <div className="overlay" onClick={closeModal}>
        <div className="modal modal-lg" onClick={e=>e.stopPropagation()}>
            ...
            {generatingEmail ? (
              ...
              <>
                ...
                <div style={{background:'#FFEB5A',border:'2px solid #191923',borderRadius:5,padding:10,fontSize:12,marginTop:8}}>
                  <strong>💡 Comment envoyer :</strong> Copie le texte ci-dessus, ouvre ton client email (Gmail, Outlook…), colle et envoie depuis <strong>emy@meshuga.fr</strong>. Une fois envoyé, clique "Contacté" dans le tableau de chasse.
                </div>
              </>
            ) : null}
          </div>
          <div className="mf">
            ...
            {generatedEmail && <>
              <button className="btn btn-y" onClick={() => {navigator.clipboard.writeText(generatedEmail);toast('Email copié ! 📋')}}>📋 Copier</button>
