...
export default function FoodCostTab(props) {
  ...
  var [ingHistory, setIngHistory] = useState([])
        ...
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.5)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={function(){setIngPopup(null)}}>
          <div style={{background:'#fff',borderRadius:12,border:'2px solid #FF82D7',padding:20,maxWidth:480,width:'100%',maxHeight:'80vh',overflowY:'auto'}} onClick={function(e){e.stopPropagation()}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              <div>
                <div style={{fontFamily:'Yellowtail',fontSize:22,color:'#191923'}}>{ingPopup.article}</div>
                <div style={{fontSize:12,color:'#FF82D7',fontWeight:700}}>{ingPopup.fournisseur} · {ingPopup.qte} {ingPopup.unite} par recette</div>
              </div>
              <button onClick={function(){setIngPopup(null)}} style={{background:'transparent',border:'none',fontSize:20,cursor:'pointer',color:'#888'}}>✕</button>
            </div>
            <div style={{display:'flex',gap:12,marginTop:16}}>
              <div style={{flex:1,background:'#FFF9D0',borderRadius:8,padding:12,textAlign:'center'}}>
                <div style={{fontSize:10,color:'#888'}}>Prix référence</div>
                <div style={{fontSize:20,fontWeight:900}}>{Number(ingPopup.prix_achat).toFixed(2)} €</div>
                <div style={{fontSize:10,color:'#888'}}>/{ingPopup.unite}</div>
              </div>
              {ingHistory.length > 0 && (
                <div style={{flex:1,background:Number(ingHistory[ingHistory.length-1].price) > ingPopup.prix_achat ? '#FFE0E0' : '#E8FFE8',borderRadius:8,padding:12,textAlign:'center'}}>
                  <div style={{fontSize:10,color:'#888'}}>Dernier prix facturé</div>
                  <div style={{fontSize:20,fontWeight:900}}>{Number(ingHistory[ingHistory.length-1].price).toFixed(2)} €</div>
                  <div style={{fontSize:10,color:'#888'}}>/{ingPopup.unite}</div>
                </div>
              )}
              {ingHistory.length > 0 && (function(){
                var last = Number(ingHistory[ingHistory.length-1].price)
                var ref = Number(ingPopup.prix_achat)
                var pct = ref > 0 ? ((last - ref) / ref * 100) : 0
                return (
            ...
            {ingHistory.length >= 2 ? (function(){
              var vals = ingHistory.map(function(h) { return Number(h.price) })
              ...
              var points = ingHistory.map(function(p, i) {
                var x = pad + (i / (ingHistory.length - 1)) * chartW
            ...
            {ingHistory.length > 0 ? ingHistory.slice().reverse().map(function(ph, i) {
              var prev = i < ingHistory.length - 1 ? ingHistory[ingHistory.length - 2 - i] : null
              ...
              return (
                <div key={i} style={{display:'flex',justifyContent:'space-between',fontSize:12,padding:'6px 0',borderBottom:'1px solid #F5F5F5',alignItems:'center'}}>
                  ...
                  <span style={{fontWeight:900}}>{Number(ph.price).toFixed(2)} €/{ingPopup.unite}</span>
